...
<img align="right" width="400" src="https://via.placeholder.com/400x250/0D47A1/FFFFFF?text=EEBCO+Building+Efficiency" alt="EEBCO">
...
<details>
<summary><b>Click to see member categories</b></summary>
...
```mermaid
graph LR
    A[EEBC Advocacy] --> B[Policy Wins]
    B --> C[Increased Funding]
    C --> D[Market Growth]
    D --> E[Job Creation]
    E --> F[Economic Impact]
    
    style A fill:#0D47A1,stroke:#FFFFFF,stroke-width:2px,color:#fff
    style B fill:#1565C0,stroke:#FFFFFF,stroke-width:2px,color:#fff
    style C fill:#1976D2,stroke:#FFFFFF,stroke-width:2px,color:#fff
    style D fill:#FF6F00,stroke:#FFFFFF,stroke-width:2px,color:#fff
    style E fill:#FF8F00,stroke:#FFFFFF,stroke-width:2px,color:#fff
    style F fill:#FFA000,stroke:#FFFFFF,stroke-width:2px,color:#fff
```
...
```mermaid
graph TD
    Board[Board of Directors] --> Exec[Executive Committee]
    Board --> Sectors[Sector Representatives]
    
    Exec --> Chair[Board Chair]
    Exec --> Vice[Vice Chair]
    Exec --> Sec[Secretary]
    Exec --> Treas[Treasurer]
    
    Sectors --> Mfg[Manufacturing & Distribution]
    Sectors --> Eng[Engineering & Architecture]
    Sectors --> Con[Contractors & Installation]
    Sectors --> ESCO[Energy Service Companies]
    Sectors --> Data[Data Analytics & Technology]
    Sectors --> Fin[Financing & Investment]
    
    Board --> ED[Executive Director]
    ED --> Staff[Professional Staff]
    ED --> Legal[Legal Counsel]
    ED --> Consult[Industry Consultants]
    
    style Board fill:#2E7D32,stroke:#1B5E20,stroke-width:2px,color:#fff
    style Exec fill:#388E3C,stroke:#2E7D32,stroke-width:2px,color:#fff
    style Sectors fill:#388E3C,stroke:#2E7D32,stroke-width:2px,color:#fff
    style ED fill:#0D47A1,stroke:#002171,stroke-width:2px,color:#fff
```
